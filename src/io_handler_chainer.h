...
public:
    ...
    IOHandlerChainer(std::shared_ptr<IOHandler> readFrom, std::shared_ptr<IOHandler> writeTo, int chunkSize);
...
private:
    ...
    char* buf;
    int chunkSize;
    std::shared_ptr<IOHandler> readFrom;
    std::shared_ptr<IOHandler> writeTo;
};
