...
static void addField(metadata_fields_t field, const TagLib::File& file, const TagLib::Tag* tag, Ref<CdsItem> item)
{
    ...
    if (string_ok(value)) {
        ...
        //        SPDLOG_TRACE(l, "Setting metadata on item: %d, %s\n", field, sc->convert(value).c_str());
    }
}
...
void TagLibHandler::fillMetadata(Ref<CdsItem> item)
{
    ...
    if (content_type == CONTENT_TYPE_MP3) {
       ...
       extractFLAC(&fs, item);
        ...
        l->warn("TagLibHandler does not handle the {} content type", content_type.c_str());
    }
    SPDLOG_TRACE(l, "TagLib handler done.\n");
}
...
void TagLibHandler::addArtworkResource(Ref<CdsItem> item, String art_mimetype)
{
    ...
    SPDLOG_TRACE(l, "Found artwork of type {} in file {}", art_mimetype.c_str(), item->getLocation().c_str());
...
void TagLibHandler::extractMP3(TagLib::IOStream *roStream, zmm::Ref<CdsItem> item) {
    ...
    if (!mp3.isValid() || !mp3.hasID3v2Tag()) {
        l->debug("TagLibHandler: could not open mp3 file: {}",
    ...
    if (aux_tags_list != nullptr) {
        for (int j = 0; j < aux_tags_list->size(); j++) {
            ...
            if (frameListMap.contains(desiredFrame.c_str())) {
                ...
                l->debug(
                    "Adding frame: {} with value {}", desiredFrame.c_str(),
                    value.c_str());
...
void TagLibHandler::extractOgg(TagLib::IOStream *roStream, zmm::Ref<CdsItem> item) {
    ...
    if (!vorbis.isValid()) {
        l->debug("TagLibHandler: could not open ogg file: {}",
                  item->getLocation().c_str());
        return;
    }
...
void TagLibHandler::extractASF(TagLib::IOStream *roStream, zmm::Ref<CdsItem> item) {
    ...
    if (!asf.isValid()) {
        l->debug("TagLibHandler: could not open asf/wma file: {}",
                  item->getLocation().c_str());
        return;
    }
...
void TagLibHandler::extractFLAC(TagLib::IOStream *roStream, zmm::Ref<CdsItem> item) {
    ...
    if (!flac.isValid()) {
        l->debug("TagLibHandler: could not open flac file: {}",
                  item->getLocation().c_str());
        return;
    }
    ...
    if (aux_tags_list != nullptr) {
        for (int j = 0; j < aux_tags_list->size(); j++) {
            ...
            if (propertyMap.contains(desiredTag.c_str())) {
                ...
                value = sc->convert(value);
                l->debug(
                    "Adding auxdata: {} with value {}", desiredTag.c_str(),
                    value.c_str());
    ...
    if (flac.pictureList().isEmpty()) {
        SPDLOG_TRACE(l, "TagLibHandler: flac resource has no picture information\n");
        return;
    }
...
void TagLibHandler::extractAPE(TagLib::IOStream *roStream, zmm::Ref<CdsItem> item) {
    ...
    if (!ape.isValid()) {
        l->debug("TagLibHandler: could not open APE file: {}",
                  item->getLocation().c_str());
        return;
    }
...
void TagLibHandler::extractWavPack(TagLib::IOStream *roStream, zmm::Ref<CdsItem> item) {
    ...
    if (!wavpack.isValid()) {
        l->debug("TagLibHandler: could not open WavPack file: {}",
                  item->getLocation().c_str());
        return;
    }
...
void TagLibHandler::extractMP4(TagLib::IOStream *roStream, zmm::Ref<CdsItem> item) {
    ...
    if (!mp4.isValid()) {
        l->debug("TagLibHandler: could not open mp4 file: {}",
                  item->getLocation().c_str());
        return;
    }

    if (!mp4.hasMP4Tag()) {
        SPDLOG_TRACE(l, "TagLibHandler: mp4 file has no tag information\n");
        return;
    }
    ...
    if (itemsListMap.contains("covr")) {
        ...
        if (coverArtList.isEmpty()) {
            SPDLOG_TRACE(l, "TagLibHandler: mp4 file has no coverart");
            return;
        }
        ...
        SPDLOG_TRACE(l, "TagLibHandler: mp4 file has no 'covr' item");
    }
}
...
void TagLibHandler::extractAiff(TagLib::IOStream *roStream, zmm::Ref<CdsItem> item) {
    ...
    if (!aiff.isValid()) {
        l->debug("TagLibHandler: could not open AIFF file: {}",
                  item->getLocation().c_str());
        return;
    }
